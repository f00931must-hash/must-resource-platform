...
function isExpired(a){
  return a.deadline && a.deadline < todayStr();
}
...
function render(){
  ...
  let data = announcements
    ...
    .filter(a => (currentCategory === "全部" || a.category === currentCategory) &&
  ...
  list.innerHTML = data.map(a => {
    ...
    return `<article class="post-card ${a.category}">
      <div class="tag-row">
        <span class="badge">${a.pinned ? "📌 置頂・" : ""}${esc(a.category)}</span>
        ${isNew(a) ? `<span class="new-badge">NEW</span>` : ""}
      </div>
      ${firstImage ? `<img class="thumb" src="${firstImage}">` : ""}
      <h3>${esc(a.title)}</h3>
      <div class="preview">${esc(a.content || "")}</div>
      <div class="meta">
  ...
  document.querySelectorAll(".open-btn").forEach(b => b.onclick = () => openModal(b.dataset.id));
}
...
function openModal(id){
  ...
  const imgs = (a.images || []).map(i => `<img class="modal-img" src="${normalizeUrl(i.url)}">`).join("");
  const files = (a.files || []).map(f => `<a class="file" href="${normalizeUrl(f.url)}" target="_blank">📎 ${esc(f.name)}</a>`).join("");

  document.getElementById("modalContent").innerHTML = `
    <span class="badge">${esc(a.category)}</span>
    ${isNew(a) ? `<span class="new-badge">NEW</span>` : ""}
    <h2>${esc(a.title)}</h2>
    <p>📅 ${esc(a.date)} ${a.deadline ? `　⏰ 截止：${esc(a.deadline)}` : ""}</p>
    ${imgs}
